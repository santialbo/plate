...
Mitigate XSS in `element.attributes` by requiring all attribute names to be allowlisted in the `dangerouslyAllowAttributes` plugin configuration option.

Migration:

For each plugin that needs to support passing DOM attributes using `element.attributes`, add the list of allowed attributes to the `dangerouslyAllowAttributes` option of the plugin.
...
To modify existing plugins, pass the `dangerouslyAllowAttributes` option to the plugin factory:
